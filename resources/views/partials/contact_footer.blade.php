...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        informtaionField = document.querySelector('.information-wrapper');
        informtaionField.classList.add('is-active');
    });
</script>

<!-- MAP -->
<script>
    // Geçerli konumları seçme
    let items = document.querySelectorAll('.information-wrapper .information-field ul .item');
    let mapChangeOverlay = document.querySelector('.mapChangeOverlay');
    let directionButton = document.querySelector('.direction-button');

    // Haritayı seçme
    let map = document.getElementById('map');

    // Varsayılan konum ve zoom düzeyi
    let latLng = {
        lat: 40.90957203742949,
        lng: 29.203623052992125
    };
    let mapZoom = 15;

    // Varsayılan harita URL'si
    let mapURL = 'https://maps.app.goo.gl/91aFgFR6ZiQJrCcc8';

    let zoomIn = document.querySelectorAll('.zoom-in');
    let zoomOut = document.querySelectorAll('.zoom-out');
    ...
    // Öğeler üzerinde dinleyicileri tanımlama
    items.forEach((item, index) => {
        item.addEventListener('click', () => {
            mapChangeOverlay.classList.add('is-active');
            setTimeout(() => {
                mapChangeOverlay.classList.remove('is-active');
            }, 500);
            items.forEach((item) => {
            ...
            let map = item.getAttribute('data-map');
            latLng = {
                lat: parseFloat(lat),
                lng: parseFloat(lng)
            };
            mapURL = map;
            directionButton.href = mapURL
            initMap();
            if (map) {
                console.log("Harita değişti: " + map);
            }
        });
    });
    ...
    // Harita marker'ı oluşturma
    let mapMarker = document.createElement('a');
    mapMarker.href = mapURL;
    mapMarker.target = '_blank';
    mapMarker.className = 'map-marker block !pointer-events-auto relative z-30';
    mapMarker.innerHTML =
        `<div class="map-marker group/map-marker">
            <div class="location-box flex justify-center z-2">
                <a href="${mapURL}" target="_blank" class="text flex justify-center group/marker">
                <svg width="25" height="35" viewBox="0 0 25 35" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M12.5 0C5.60746 0 0 5.68393 0 12.6704C0 21.3408 11.1863 34.0695 11.6625 34.6071C12.1099 35.1121 12.8909 35.1113 13.3375 34.6071C13.8137 34.0695 25 21.3408 25 12.6704C24.9999 5.68393 19.3925 0 12.5 0ZM12.5 19.0452C9.03219 19.0452 6.211 16.1855 6.211 12.6704C6.211 9.15529 9.03226 6.29563 12.5 6.29563C15.9677 6.29563 18.7889 9.15536 18.7889 12.6705C18.7889 16.1856 15.9677 19.0452 12.5 19.0452Z" fill="#75BF00"/>
                </svg>
                </a>
            </div>
        </div>`;

    // Harita marker'ını gösterme
    window.initMap();
</script>
